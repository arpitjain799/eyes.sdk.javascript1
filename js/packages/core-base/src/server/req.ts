...
export function makeReq(baseOption?: Options): typeof req {
  const customReq = (location, options) => req(location, mergeOptions(baseOption, options))
  customReq.stop = stop
  return customReq
}
...
export async function req(input: string | URL | Request, options?: Options): Promise<Response> {
  ...
  try {
    ...
    const retry = (options?.retry as Retry[])?.find(
      retry => retry.codes?.includes(error.code) && (!retry.limit || !retry.attempt || retry.attempt < retry.limit),
    )
    if (retry) {
      retry.attempt ??= 0
      ...
      const retryRequest = await beforeRetry({request: request.clone(), error, attempt: retry.attempt, options})
      if (retryRequest !== stop) {
        return req(retryRequest, options)
      }
    }
